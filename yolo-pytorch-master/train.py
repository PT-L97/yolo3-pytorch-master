...
if __name__ == "__main__":
    ...
    model_path      = 'model_data/yolo_weights.pth'#一般来讲是coco的预训练权重
    #------------------------------------------------------#
    #   输入的shape大小，一定要是32的倍数
    #------------------------------------------------------#
    input_shape     = [416, 416]#进行训练的图片大小
    ...
    Freeze_Epoch        = 50
    Freeze_batch_size   = 8
    Freeze_lr           = 1e-3#一般冻结是50个世代，bs是8，显存大调大
    #----------------------------------------------------#
    #   解冻阶段训练参数
    #   此时模型的主干不被冻结了，特征提取网络会发生改变
    #   占用的显存较大，网络所有的参数都会发生改变--所训练的权值变大了。
    #----------------------------------------------------#
    UnFreeze_Epoch      = 100
    ...
    if True:
        ...
        if epoch_step == 0 or epoch_step_val == 0:
            raise ValueError("Dataset too small, please expand dataset")
